...
description: "Google Tag Manager에서 노티플라이 템플릿으로 SDK 초기화, 유저/이벤트 연동을 설정하는 방법을 설명합니다."
---
...
<Info>
- 이메일 발송에는 **`$email`**, 카카오 알림톡/친구톡/문자 발송에는 **`$phone_number`**, LINE 메시지 발송에는 **`$line_user_id`** 예약 키가 필요합니다.
- 예약 키는 반드시 표기 그대로 사용하세요. 예: `$email`, `$phone_number`, `$line_user_id`
</Info>
