---
title: "iOS"
description: "노티플라이 iOS SDK 설치와 AppDelegate 설정, 푸시 알림 연동 방법을 안내합니다."
---
...
<AccordionGroup>
  ...
  <Accordion title="SwiftUI 프로젝트에서 푸시 알림 토큰이 전달되지 않거나 인앱 팝업이 작동하지 않습니다.">
    SwiftUI 환경에서 Firebase SDK의 method swizzling이 delegate 메서드를 올바르게 전달하지 못할 수 있습니다.
    ([관련 이슈: firebase-ios-sdk#7473](https://github.com/firebase/firebase-ios-sdk/issues/7473))

    Info.plist에 `FirebaseAppDelegateProxyEnabled`를 `NO`(Boolean)로 설정하여 swizzling을 비활성화하세요.
    ...
    Swizzling 비활성화 후에는 APNs 토큰을 Firebase에 수동으로 전달해야 합니다:
