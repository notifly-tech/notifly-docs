...
Amplitude 코호트 동기화는 Amplitude에서 분석을 통해 생성한 유저 그룹을 노티플라이의 유저 그룹으로 자동 동기화하는 기능입니다.
Amplitude 공식 문서 - [Notifly (Cohort Sync)](https://amplitude.com/docs/data/destination-catalog/notifly-cohort-sync)를 참고하세요.
...
코호트 동기화를 설정하기 전에 다음 사항들을 확인해 주세요:
  ...
  - 노티플라이는 코호트당 하나의 유저 그룹을 생성합니다. 코호트 불일치를 방지하기 위해 동일한 코호트에 대한 각 새 동기화는 기존 유저 그룹을 재설정합니다. (주의: "새 동기화"는 새로 추가된 destination을 의미합니다.) 다른 매핑으로 다시 동기화할 때 주의하세요.
...
3. Destination 이름을 입력하고, 노티플라이에서 복사한 Project ID, Access Key, Secret Key를 입력합니다.
